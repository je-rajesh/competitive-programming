#include<bits/stdc++.h>
// if you have problems with this code c++,
// consult your programming text book first...
...
double matching(int bitmask)
{
    ...
    double ans = 2000000000.0; // initialise with a large value
    int p1, p2;

    for (p1 = 0; p1 < 2 * N; p1++)
    {
        if (!(bitmask & (1 << p2))) //
            break;                  // find the first bit that if off
    }

    for (p2 = 0; p2 < 2 * N; p2++)
    {
        if (!(bitmask & (1 << p2))) // then, try to match p1
...
int main()
{
    ...
    //freopen("10911.txt", "r", stdin); redirect cin file to stdin
